...
import de.yochyo.yummybooru.layout.activities.fragments.tagHistoryFragment.recyclerview.TagFragmentViewHolder
...
class TagHistoryTagAdapterUtil(val fragment: TagHistoryTagFragment, val adapter: RecyclerView.Adapter<*>) {
    ...
    fun onCreateViewHolder(parent: ViewGroup, viewType: Int): TagFragmentViewHolder =
        TagFragmentViewHolder((LayoutInflater.from(fragment.ctx).inflate(R.layout.search_item_layout, parent, false) as Toolbar)).apply {
            ...
            fun onClick() {
                fragment.viewModel.selectedTags.value =
                    if (check.isChecked) fragment.viewModel.selectedTagsValue.value.addToCopy(toolbar.findViewById<TextView>(R.id.search_textview).text.toString())
    ...
    fun onBindViewHolder(holder: TagFragmentViewHolder, position: Int) {
        ...
        Menus.initMainSearchTagMenu(fragment.ctx, holder.toolbar.menu, tag)
    }
}