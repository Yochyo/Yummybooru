...
class Subscription(@PrimaryKey val name: String, val type: Int, var last: Int, var current: Int = last, val isFavorite: Boolean = false, val creation: Date = Date()) : Comparable<Subscription> {
    val color: Int
        get() {
            when (type) {
